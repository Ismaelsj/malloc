...
/*   Updated: 2020/10/15 19:29:17 by user42           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_area		*append_new_area(t_area *prev, size_t size)
{
	...
	if ((new_area = request_memory(get_page_size(prev->type, size))) == NULL)
		return (NULL);
...
t_area		*request_memory(size_t size)
{
	t_area *new_alloc;

	new_alloc = NULL;
	if ((new_alloc = mmap(0, size, PROT_READ | PROT_WRITE | PROT_EXEC,\
